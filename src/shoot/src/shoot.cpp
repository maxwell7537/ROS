...
public:
    shoot(int id=1):Node("shoot_node"){
        ...
        // 启用「相对鼠标模式」：核心配置，适合视角控制
        SDL_SetRelativeMouseMode(SDL_TRUE);
        // 相对鼠标模式特点：
        // 1. 隐藏鼠标光标，避免光标移出屏幕导致控制中断；
        // 2. 鼠标移动仅返回「相对位移」（xrel/yrel），而非绝对坐标；
        // 3. 适合云台、游戏视角等需要连续控制的场景。
        is_mouse_captured_=true;// 标记鼠标已成功捕获
